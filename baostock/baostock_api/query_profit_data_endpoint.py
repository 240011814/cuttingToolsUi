...
def parse_params(query: dict[str, list[str]]) -> QueryProfitDataParams:
    return QueryProfitDataParams(
...
def execute(params: QueryProfitDataParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_profit_data(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_profit_data failed")
