...
class QueryOperationDataParams:
    code: str
    year: str | None
    quarter: str | None
...
@dataclass(frozen=True)
class QueryOperationDataResult:
    code: str
    year: str | None
    quarter: str | None
    fields: list[str]
...
def parse_params(query: dict[str, list[str]]) -> QueryOperationDataParams:
    return QueryOperationDataParams(
...
def execute(params: QueryOperationDataParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_operation_data(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_operation_data failed")
    ...
    payload = QueryOperationDataResult(
        code=params.code,
        year=params.year,
