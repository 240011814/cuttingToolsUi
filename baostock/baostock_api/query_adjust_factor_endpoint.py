...
def parse_params(query: dict[str, list[str]]) -> QueryAdjustFactorParams:
    return QueryAdjustFactorParams(
...
def execute(params: QueryAdjustFactorParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_adjust_factor(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_adjust_factor failed")
    ...
    payload = QueryAdjustFactorResult(
        code=params.code,
