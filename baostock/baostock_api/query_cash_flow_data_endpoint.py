...
def parse_params(query: dict[str, list[str]]) -> QueryCashFlowDataParams:
    return QueryCashFlowDataParams(
...
def execute(params: QueryCashFlowDataParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_cash_flow_data(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_cash_flow_data failed")
    ...
    payload = QueryCashFlowDataResult(
        code=params.code,
        year=params.year,
