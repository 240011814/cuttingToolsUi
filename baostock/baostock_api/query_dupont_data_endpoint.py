...
def parse_params(query: dict[str, list[str]]) -> QueryDupontDataParams:
    return QueryDupontDataParams(
...
def execute(params: QueryDupontDataParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_dupont_data(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_dupont_data failed")
