...
def parse_params(query: dict[str, list[str]]) -> QueryBalanceDataParams:
    return QueryBalanceDataParams(
...
def execute(params: QueryBalanceDataParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_balance_data(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_balance_data failed")
    ...
    payload = QueryBalanceDataResult(
        code=params.code,
        year=params.year,
