...
class QueryForecastReportParams:
    code: str
    start_date: str | None
    end_date: str | None
...
@dataclass(frozen=True)
class QueryForecastReportResult:
    code: str
    start_date: str | None
    end_date: str | None
    fields: list[str]
...
def parse_params(query: dict[str, list[str]]) -> QueryForecastReportParams:
    return QueryForecastReportParams(
...
def execute(params: QueryForecastReportParams) -> dict[str, object]:
    query_kwargs = {
        "code": params.code,
    }
    ...
    with baostock_lock:
        ...
        try:
            result = bs.query_forecast_report(**query_kwargs)

            if result.error_code != "0":
                raise BaostockQueryError(result.error_msg or "query_forecast_report failed")
    ...
    payload = QueryForecastReportResult(
        code=params.code,
