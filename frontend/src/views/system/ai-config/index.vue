...
const modelDialogTitle = computed(() => (modelModalMode.value === 'list' ? $t('page.system.aiConfig.manageModel') : modelModalTitle.value));
...
const columns = computed<DataTableColumns<Api.Admin.AIProvider>>(() => [
  ...
  },
  {
    ...
    render(row) {
      return h(NSpace, {}, {
        default: () => [
          h(NButton, { size: 'small', onClick: () => handleEditProvider(row) }, { default: () => $t('common.edit') }),
          h(NButton, { size: 'small', type: 'primary', ghost: true, onClick: () => handleManageModels(row) }, { default: () => $t('page.system.aiConfig.manageModel') }),
...
const modelColumns = computed<DataTableColumns<Api.Admin.AIModel>>(() => [
  ...
  { title: $t('page.system.aiConfig.displayName'), key: 'display_name' },
  {
    ...
    key: 'is_default',
    render(row) {
      return row.is_default ? h(NTag, { type: 'success' }, { default: () => $t('page.system.aiConfig.default') }) : null;
    }
  },
  {
    title: $t('page.system.aiConfig.actions'),
    ...
    render(row) {
      return h(NSpace, {}, {
        default: () => [
          h(NButton, { size: 'small', onClick: () => handleEditModel(row) }, { default: () => $t('common.edit') }),
...
<template>
  <div class="h-full p-4">
    <NCard :title="$t('page.system.aiConfig.title')" :bordered="false" class="h-full rounded-16px shadow-sm">
      <template #header-extra>
        <NButton type="primary" @click="handleAddProvider">{{ $t('page.system.aiConfig.addProvider') }}</NButton>
      </template>
    ...
    <NModal v-model:show="showProviderModal" :title="providerModalTitle" preset="card" class="w-500px">
      <NForm :model="providerForm" label-placement="left" :label-width="100">
        <NFormItem :label="$t('page.system.aiConfig.providerName')" path="name">
          <NInput v-model:value="providerForm.name" :placeholder="$t('page.system.aiConfig.providerNamePlaceholder')" />
        </NFormItem>
        <NFormItem label="API Key" path="api_key">
          <NInput v-model:value="providerForm.api_key" type="password" show-password-on="click" :placeholder="$t('page.system.aiConfig.apiKeyPlaceholder')" />
        </NFormItem>
        <NFormItem label="Base URL" path="base_url">
          <NInput v-model:value="providerForm.base_url" :placeholder="$t('page.system.aiConfig.baseUrlPlaceholder')" />
        </NFormItem>
        ...
        <div class="flex justify-end gap-2">
          ...
          <NButton type="primary" @click="handleSaveProvider">{{ $t('common.confirm') }}</NButton>
        </div>
      </NForm>
    </NModal>
    ...
    <NModal v-model:show="showModelModal" :title="modelDialogTitle" preset="card" :class="modelModalMode === 'list' ? 'w-800px' : 'w-500px'">
      <div v-if="modelModalMode === 'list'">
        <div class="mb-4 flex justify-end">
          <NButton type="primary" size="small" @click="handleAddModel">{{ $t('page.system.aiConfig.addModel') }}</NButton>
        </div>
      ...
      <NForm v-else :model="modelForm" label-placement="left" :label-width="100">
        <NFormItem :label="$t('page.system.aiConfig.modelCode')" path="model_code">
          <NInput v-model:value="modelForm.model_code" :placeholder="$t('page.system.aiConfig.modelCodePlaceholder')" />
        </NFormItem>
        <NFormItem :label="$t('page.system.aiConfig.displayName')" path="display_name">
          <NInput v-model:value="modelForm.display_name" :placeholder="$t('page.system.aiConfig.displayNamePlaceholder')" />
        </NFormItem>
        ...
        <div class="flex justify-end gap-2">
          ...
          <NButton type="primary" @click="handleSaveModel">{{ $t('common.confirm') }}</NButton>
        </div>
      </NForm>
    </NModal>
  </div>
